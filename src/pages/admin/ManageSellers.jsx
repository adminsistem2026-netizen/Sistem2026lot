              ...
              <div className="mt-3 pl-3 border-l-2 border-violet-500/30 space-y-1.5">
                {ownedSellers.length === 0 ? (
                  ...
                  <div key={sv.id} className="flex items-center gap-2">
                    <span className="text-xs text-slate-300">{sv.full_name}</span>
                    <span className="text-xs text-slate-500">{sv.email}</span>
                    <span className={`text-xs px-1.5 py-0.5 rounded-full ${sv.is_active ? 'text-emerald-400' : 'text-slate-500'}`}>
                      {sv.is_active ? '●' : '○'}
                    </span>
                  </div>
                ))}

    ...
    private byte[] buildEscPosTicket(String ticketJson) throws Exception {
        ...
        if (numbers != null) {
            for (int i = 0; i < numbers.length(); i++) {
                ...
                bos.write(new byte[]{0x1B, 0x20, 0x04});                    // +4 puntos espacio entre chars
                bos.write(new byte[]{0x1D, 0x21, 0x01, 0x1B, 0x45, 0x01}); // doble alto + bold
                writeStr(bos, padRight(num, 12) + padCenter(pcs, 10) + padLeft(sub, 10) + "\n");
                bos.write(new byte[]{0x1D, 0x21, 0x00, 0x1B, 0x45, 0x00});
                bos.write(new byte[]{0x1B, 0x20, 0x00});                    // reset spacing
                writeStr(bos, "--------------------------------\n");
            }
        }
